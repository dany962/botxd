...
client.on("guildMemberAdd", (member) => {
    ...
    var guild = member.guild, fake = (Date.now() - member.createdAt) / (1000 * 60 * 60 * 24) <= 3 ? true : false, channel = guild.channels.cache.get('815630022928891905');
...
client.on("guildMemberRemove", (member) => {
    ...
    var total = 0, bonus = 0, regular = 0, fakecount = 0, channel = member.guild.channels.cache.get('815630022928891905'), content = settings.leaveMessage ? settings.leaveMessage : `${member} is left the server.`, data = db.get(`invites.${member.id}`);
    if(!data){
        if(channel){
            content = content
            .replace("-member-", `${member}`);
